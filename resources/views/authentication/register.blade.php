...
@section('overlay')
  <form method="POST" action="/register"
        class="m-5 w-full space-y-6 rounded-xl bg-sky-800 p-5 shadow-xl sm:w-3/5 lg:w-2/5 xl:w-1/3">
    @csrf
    ...
    <main class="flex flex-col space-y-2 font-thin">
      ...
      <x-form.dropdown name="role_id" label="Role" :items="$roles" />
    </main>

    <footer class="flex flex-col items-center space-y-1">
      <x-form.button type="submit" text="Register" class="w-2/3" />
      <a href="/login" class="text-violet-200 hover:opacity-90 active:opacity-80">Already have an account?</a>
    </footer>
  </form>
@endsection
