<div x-data="flash_message" x-show="open" x-on:click.outside="open = !open " x-init="timer = setTimeout(() => open = false, 2000)"
     ...
     class="{{ session('success') ? 'bg-green-100 dark:bg-green-200' : 'bg-red-100 dark:bg-red-200' }} absolute left-1/2 top-5 z-10 flex w-4/5 -translate-x-1/2 rounded-lg p-4 drop-shadow-lg sm:w-4/5 lg:w-3/5 xl:w-2/3">
    <svg aria-hidden="true"
         class="{{ session('success') ? 'text-green-700 dark:text-green-800' : 'text-red-700 dark:text-red-800' }} h-5 w-5 flex-shrink-0"
